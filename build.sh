...
# Membaca token bot dan chat ID dari variabel lingkungan
token="$token"
chat_id="$chat_id"

# Memeriksa apakah variabel lingkungan telah diatur
if [ -z "$token" ] || [ -z "$chat_id" ]; then
  echo "Error: Token bot atau chat ID tidak diatur."
  exit 1
fi
...
export CODENAME="selenes"
export DEVICE="Redmi 10 4G/Redmi Note 11 4G (${CODENAME})"
...
export zipn="Hydrogen-${CODENAME}-${TIMESTAMP}"
# Needed by script
...
if [ ! -d "${PWD}/anykernel" ]; then
    ...
    git clone -b Hydrogen https://github.com/noticesax/AnyKernel3 anykernel
    if [ $? -ne 0 ]; then
...
zip_kernel() {
    ...
    # Clean up anykernel directory
    if [ ! -f "./out/arch/${ARCH}/boot/Image.gz-dtb" ]; then
        rm -f ./anykernel/Image.gz
    else
        rm -f ./anykernel/Image.gz-dtb
    fi
    rm -f ./anykernel/dtbo.img
